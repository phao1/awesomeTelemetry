...
- [x] 1.5 验收：启动后直接点 compare，两个选择器已填充，选中两条能出结果
...
- [x] 2.6 验收：`kill` 掉后端后点击任意会话，5 秒内出现错误码 + 重试按钮，状态栏变断开
- [x] 2.7 验收：loading 骨架行高 == 真实行高，数据到达时 CLS = 0
...
- [x] 3.5 验收：状态栏**不轮询** `/api/health`；localStorage 塞脏值不崩溃
...
- [x] 4.4 验收：500 条会话滚动掉帧 < 5%；行高恒为 `--row-lg`（G-DS-1）
...
- [x] 5.9 验收：9,590 event 会话 DOM < 500、首绘 < 200ms

## 6. Agent 概览（T-20，spec REQ-018）

- [x] 6.1 顶部 4 张 KPI 卡（快/准/稳/省 加权聚合）
- [x] 6.2 可排序对比表 + 内联 BarMeter / Sparkline，数值列右对齐 mono
- [x] 6.3 `null` 与 `0` 视觉可区分
- [x] 6.4 行展开列出该 provider 最近 10 条会话，点击跳转并选中
- [x] 6.5 验收：切到该视图网络请求数 **== 1**；展开行不产生新请求（G11.9）

## 7. 对比视图（T-21，spec REQ-019）

- [x] 7.1 搜索式会话选择器（Popover + 搜索，**不用原生 select**）
- [x] 7.2 **结论条**：一句话给出「谁快多少 / 谁省多少」
- [x] 7.3 四维双条对比，按「谁更优」着色；L/R 除颜色外必须有字母标记
- [x] 7.4 PhaseRibbon 上下对照（共享同一时间比例尺）+ 时间线并排
- [x] 7.5 未选择时给 EmptyState 引导，不是两个空下拉

## 8. Proxy / Frida 视图（T-22，spec REQ-020/021）

- [x] 8.1 Proxy 控制条：状态 / 启动停止 / 端口 / CA 证书下载 / 请求计数与清空
- [x] 8.2 停止与清空是破坏性操作，需二次确认（design-system REQ-007）
- [x] 8.3 请求列表密排表 + 方法色标 + 状态码语义色 + 过滤
- [x] 8.4 详情抽屉：Request / Response / Headers / Timing 分页签，脱敏字段标注
- [x] 8.5 Frida 控制条 + 捕获列表 + 前置条件未满足时说清缺什么怎么装
- [x] 8.6 验收：未启动时空态含启动按钮与 CA 安装提示，不是一句「暂无数据」

## 9. 阶段收口

- [x] 9.1 `npm run typecheck && npm run test && npm run lint` 全绿
- [x] 9.2 五个视图逐一手工走查，无 console error
- [x] 9.3 更新 `PROGRESS.md`
